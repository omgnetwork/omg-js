...
class RootChain {
  ...
  async depositEth(depositTx, amount, txOptions, callbacks) {
    ...
    const txDetails = {
      ...
      to: ethVaultAddress,
      value: amount,
      data: txUtils.getTxData(this.web3, ethVaultContract, 'deposit', depositTx)
    };
  ...
  async processExit(exitId, token, txOptions) {
    const paymentExitGameContractAddress = await this.plasmaContract.methods
      .exitGames(PAYMENT_TX_TYPE)
      .call();

    const paymentExitGameContract = this.getContract(
      this.paymentExitGameAbi.abi,
      paymentExitGameContractAddress
    );
